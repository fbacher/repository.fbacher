...
HASH_TYPES: List[str] = ['md5', 'sha256', 'sha512', 'sha1']
ZIP_HASH_TYPE: str = 'sha256'
...
def generate_checksum(archive_path, hash_type: str=None):
    ...
    with io.open(f'{archive_path}.{hash_type}', 'wt') as f:
        f.write(digest.hexdigest())

    print(f'{archive_path} hash_type: {hash_type} sum: {digest.hexdigest()}')
    ...
    try:
        ...
        with open(archive_path, 'wb') as archive:
            cloned.archive(
                ...
                format='zip')
        generate_checksum(archive_path, hash_type=ZIP_HASH_TYPE)
